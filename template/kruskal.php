...
function PrintResult($result, $e)
{
	...
	global $sedge, $dedge, $label, $edge;
	for ($i = 0; $i < $e; ++$i)
	{
		$sedge[$i+1] = $result[$i]->Source+1;
		$dedge[$i+1] = $result[$i]->Destination+1;
		$label[$i+1] = $result[$i]->Weight;
	}
	$edge = $e;
}
...
$graph = CreateGraph($v, $edge);
for($i = 1; $i <= $edge; $i++)
{
	$graph->_edge[$i-1]->Source = $sedge[$i]-1;
	$graph->_edge[$i-1]->Destination = $dedge[$i]-1;
	$graph->_edge[$i-1]->Weight = $label[$i];
}

//untuk keperluan menampilkan list edge lama secara terurut
usort($graph->_edge, "CompareEdges");
for($i = 1; $i <= $edge; $i++)
{
	$old_sedge[$i] = $graph->_edge[$i-1]->Source + 1;
	$old_dedge[$i] = $graph->_edge[$i-1]->Destination + 1;
	$old_label[$i] = $graph->_edge[$i-1]->Weight;
}
